...
**Read them through `lang/golang`, never through `lang/golang/frontend`.**
...
The two live in one module, which is a distinction worth holding: a
plugin requiring `go.thesmos.sh/eidos/lang/golang` gets the module
that also contains this frontend, and may still not import the
frontend package. The rule is about what a plugin *imports*, not what
its `go.mod` lists — Go compiles only the packages actually reached,
and depguard fails the build on the import rather than the
requirement.
...
Stamping happens in `lang/golang/frontend/stamp.go` via per-kind helpers:
