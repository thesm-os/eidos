...
// through [storefixture.Builder.PackageNode].
//
...
// handing the store to a plugin. Use [storefixture.Builder.Build]
...
func Example() {
	pkg := storefixture.New().
		...
		Struct("User", func(s *storefixture.StructBuilder) {
			...
			s.Directive(storefixture.Directive("repo", storefixture.KV("table", "users")))
			s.Field("ID", storefixture.Named("string"), func(f *storefixture.FieldBuilder) {
				f.Tag(`json:"id"`)
			})
			s.Field("Roles", storefixture.Slice(storefixture.Named("string")), nil)
		}).
		PackageNode()
