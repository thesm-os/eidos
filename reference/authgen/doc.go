...
// Package authgen is a reference contributor: a plugin that renders into
// another plugin's file without owning one.
//
// It contributes the authentication entry, and is the simplest form: no dependencies within its bucket, so it renders first.
//
...
//
// See [go.thesmos.sh/eidos/reference/metricgen] for the companion that orders itself after this one.
package authgen
