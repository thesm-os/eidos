...
- **`pagination`'s `cursor=` resolves as a value field** ([#83](https://github.com/thesm-os/eidos/issues/83)) rather than staying opaque, so a continuation claim can select the member off the page the reader answered instead of only reading its name — **a name the answered page does not carry is now reported**.
- **`transaction` accepts `write=` and `read=`** ([#82](https://github.com/thesm-os/eidos/issues/82)), both `shape.KindCallable` scoped to the `fn` role: `notfound=` names the error a read reports for work the rollback undid and the contract named neither the read nor the write it reads after — and neither is derivable, since the scope's whole signature is the body it runs. Optional.
- **`shape.Param` can be marked `Counterexample`, and `ids.Counterexamples` enumerates the marked keys** across mixins and contracts: the five declared-adversarial-input params (`invalid=`, `unsafe=` ×2, `edge=`, `refused=`) share one story, and the marker lets a coverage gate ask it once instead of remembering five spellings. Declarative — the validator does not read it.
- **`batch-writer` accepts `refused=`, naming a declared value the writer turns down** ([#81](https://github.com/thesm-os/eidos/issues/81)). `shape.KindVar` scoped to the writer role: `mode=atomic`'s check is write-refused-then-read-absent, and the `reader` role supplied the read while nothing supplied a write a derived draw could not — a draw is built to be accepted. Optional.
- **`golang.InterfaceOf` and `golang.MemberMethod`** ([#80](https://github.com/thesm-os/eidos/issues/80)) resolve a type reference to the interface declaring it and find an exported method on its full method set, embedding walked and shadowing honoured — the mirror of `StructOf` / `MemberField` for the handle a `cursor` or `watcher` producer answers, and the resolution `shape.KindMember` promises. Forwarded from `lang/golang/sdk` from the start.
- **`xsssafe` and `injectionsafe` accept `unsafe=`, and `total` accepts `edge=`** ([#79](https://github.com/thesm-os/eidos/issues/79)) — a declared value carrying what must not survive, or an input at the domain's boundary: the witness no derivation can produce, `validates invalid=` generalised to the three classifications with the same trap. All `shape.KindVar`, all optional.
- **`validates` accepts `invalid=`, naming a declared value the validator refuses** ([#78](https://github.com/thesm-os/eidos/issues/78)). `shape.KindVar`, optional: without it the only stateable law binds and cannot fail — the corpus proved it by deleting the subject's screening and watching the check stay green.
