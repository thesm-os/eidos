...
//
// An entry whose file is already absent is counted separately and
// reported as such. Both other branches would have called it a
// deletion — the dry run said "would delete", the real run said
// "deleted" — for a file neither had anything to do with, which is
// a report a reader has to verify by hand before believing.
func (c *PruneCommand) applyPrune(
	...
	for _, o := range stale {
		...
		if _, statErr := os.Stat(fullPath); errors.Is(statErr, os.ErrNotExist) {
			// Claimed by the manifest and already gone. Reported
			// before the marker check because both later branches
			// would call it a deletion: the dry run said "would
			// delete" and the real run said "deleted", for a file
			// neither had anything to do with. The entry still
			// clears from the manifest, which is the half that was
			// always working.
			fmt.Fprintf(env.Stdout, "already gone: %s\n", fullPath)
			gone++
			continue
		}
		...
		if !ok {
			// Present without the generated marker — someone edited
			// it, or it was never ours. The missing case cannot
			// reach here: it is answered above.
			if env.Diag != nil {
	...
	if c.Config.DryRun {
		fmt.Fprintf(env.Stdout, "prune dry-run: %d would-delete, %d already gone, %d skipped\n",
			deleted, gone, skipped)
	} else {
		fmt.Fprintf(env.Stdout, "prune: %d deleted, %d already gone, %d skipped\n",
			deleted, gone, skipped)
