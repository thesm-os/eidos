...
func TestFrontendName(t *testing.T) {
	...
	t.Run("matches the canonical plugin name", func(t *testing.T) {
		t.Parallel()
		if frontend.FrontendName != "golang" {
			t.Fatalf("FrontendName = %q, want %q", frontend.FrontendName, "golang")
		}
	})
}
...
func TestFrontend_Version(t *testing.T) {
	...
	t.Run("returns FrontendVersion", func(t *testing.T) {
		t.Parallel()
		if got := frontend.New().Version(); got != frontend.FrontendVersion {
			t.Fatalf("Version = %q, want %q", got, frontend.FrontendVersion)
		}
	})
}
...
func TestFrontend_EmitVersions(t *testing.T) {
	...
	t.Run("contains the in-tree emit major", func(t *testing.T) {
		t.Parallel()
		majors := frontend.New().EmitVersions()
		want := emit.Major()
...
func benchmarkFrontend(b *testing.B, dir string, ignoreWorkspace bool) *frontend.Frontend {
	b.Helper()
	fe := frontend.New()
...
func assertBenchmarkLoadConverts(b *testing.B, fe *frontend.Frontend, parser *directive.Parser, wantStructs int) {
	b.Helper()
