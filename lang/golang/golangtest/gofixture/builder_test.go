...
func TestBuilder_Package(t *testing.T) {
	...
	t.Run("retargets the file on members, not only on their owners", func(t *testing.T) {
		t.Parallel()
		// A member inherits its owner's synthetic file, so a rename
		// that moved the struct and left its fields behind would route
		// two halves of one declaration into two directories — and
		// Layout reads the member's position, not the owner's.
		p := gofixture.New().
			Struct("S", func(sb *gofixture.StructBuilder) {
				sb.Field("ID", gofixture.Named("string"), nil)
				sb.Method("Validate", nil)
			}).
			Interface("I", func(ib *gofixture.InterfaceBuilder) {
				ib.Method("Read", nil)
			}).
			Package("users", "example.com/users").
			PackageNode()
		...
		for _, got := range []string{
			p.Structs[0].Fields[0].SourcePos.File,
			p.Structs[0].Methods[0].SourcePos.File,
			p.Interfaces[0].Methods[0].SourcePos.File,
		} {
			if !strings.HasPrefix(got, "users/") {
				t.Errorf("member file = %q, want it under the renamed package", got)
			}
		}
	})
...
// TestBuilder_SyntheticFileFallbacks covers the two names the
// synthetic source file falls back to.
//
// Neither is a shape a fixture should be in — an unnamed declaration
// and a package with no name are both misuse — but the fallback exists
// so misuse does not reintroduce the empty basename the position is
// there to prevent. A file called ".go" routes generated output to a
// name the Go toolchain discards without saying so.
func TestBuilder_SyntheticFileFallbacks(t *testing.T) {
	t.Parallel()

	t.Run("an unnamed declaration keeps a non-empty stem", func(t *testing.T) {
		t.Parallel()
		got := gofixture.New().Struct("", nil).PackageNode().Structs[0].SourcePos.File
		if got != "test/decl.go" {
			t.Fatalf("file = %q, want test/decl.go", got)
		}
	})

	t.Run("a package with no name drops the directory", func(t *testing.T) {
		t.Parallel()
		got := gofixture.New().
			Package("", "example.com/users").
			Struct("S", nil).
			PackageNode().Structs[0].SourcePos.File
		if got != "s.go" {
			t.Fatalf("file = %q, want a bare s.go rather than a leading separator", got)
		}
	})
}
...
func TestBuilder_Import(t *testing.T) {
	...
	t.Run("records the import on the package", func(t *testing.T) {
		t.Parallel()
		b := gofixture.New().Import("context").Import("time")
...
func TestBuilder_Build(t *testing.T) {
	...
	t.Run("returns a populated store with the configured package", func(t *testing.T) {
		t.Parallel()
		s := gofixture.New().Struct("User", nil).Build()
	...
	t.Run("panics on duplicate qualified names", func(t *testing.T) {
		t.Parallel()
		b := gofixture.New().Struct("Dup", nil).Struct("Dup", nil)
	...
	t.Run("an explicit Pos call wins over the synthetic default", func(t *testing.T) {
		...
		pkg := gofixture.New().
			Struct("User", func(s *gofixture.StructBuilder) {
				...
				s.Method("Save", func(m *gofixture.MethodBuilder) { m.Pos(pos) })
			}).
			Interface("Store", func(i *gofixture.InterfaceBuilder) { i.Pos(pos) }).
			Function("Run", func(f *gofixture.FunctionBuilder) { f.Pos(pos) }).
			Variable("Default", func(v *gofixture.VariableBuilder) { v.Pos(pos) }).
			Constant("Max", func(c *gofixture.ConstantBuilder) { c.Pos(pos) }).
			Enum("Colour", func(e *gofixture.EnumBuilder) { e.Pos(pos) }).
			Alias("Key", func(a *gofixture.AliasBuilder) { a.Pos(pos) }).
			PackageNode()
...
// [gofixture.Builder.Package] cannot express.
