...
func projectionFixture() *gofixture.Builder {
	return gofixture.New().
		Package("users", "example.com/app/users").
		Alias("UserID", func(a *gofixture.AliasBuilder) {
			a.Docs("UserID identifies a user.").Target(gofixture.Named("string")).True()
		}).
		Alias("Counter", func(a *gofixture.AliasBuilder) {
			a.Target(gofixture.Named("int"))
		}).
		Enum("Status", func(e *gofixture.EnumBuilder) {
			e.Underlying(gofixture.Named("int")).
				Variant("StatusDraft", "").
				Variant("StatusLive", "", func(v *gofixture.VariantBuilder) {
		...
		Struct("User", func(s *gofixture.StructBuilder) {
			...
			s.Embed(gofixture.PkgNamed("sync", "Mutex"))
			s.Field("ID", gofixture.Named("UserID"), func(f *gofixture.FieldBuilder) {
				f.Docs("ID is the primary key.").Tag(`json:"id"`)
			})
			s.Field("Tags", gofixture.Slice(gofixture.Named("string")), nil)
			s.Field("Meta", gofixture.Map(
				gofixture.Named("string"), gofixture.Named("any"),
			), nil)
			...
			s.Field("Events", gofixture.RecvChan(gofixture.Named("Status")), nil)
			s.Field("Ticks", gofixture.SendChan(
				gofixture.PkgNamed("time", "Time"),
			), nil)
		...
		Struct("List", func(s *gofixture.StructBuilder) {
			s.TypeParam("T", nil)
			s.Field("Items", gofixture.Slice(gofixture.TypeParamRef("T")), nil)
			s.Method("First", func(m *gofixture.MethodBuilder) {
				m.Return(gofixture.TypeParamRef("T"))
			})
		}).
		Interface("Store", func(i *gofixture.InterfaceBuilder) {
			i.Embed(gofixture.PkgNamed("io", "Closer"))
			i.Method("Get", func(m *gofixture.MethodBuilder) {
				m.Param("ctx", gofixture.PkgNamed("context", "Context"))
				m.Param("id", gofixture.Named("string"))
				m.NamedReturn("item", gofixture.Pointer(gofixture.Named("User")))
				m.NamedReturn("err", gofixture.Named("error"))
			})
		...
		Struct("Sum", func(s *gofixture.StructBuilder) {
			// A type-set bound, which only [gofixture.Bound] can
			...
			s.TypeParam("N", gofixture.Bound("~int | ~float64"))
			s.Field("Total", gofixture.TypeParamRef("N"), nil)
		}).
		Constant("MaxUsers", func(c *gofixture.ConstantBuilder) {
			c.Type(gofixture.Named("int")).Value("100")
		}).
		Variable("Registry", func(v *gofixture.VariableBuilder) {
			v.Type(gofixture.Map(gofixture.Named("string"),
				gofixture.Pointer(gofixture.Named("User"))))
		}).
		Function("New", func(f *gofixture.FunctionBuilder) {
			f.Param("id", gofixture.Named("string"))
			f.Return(gofixture.Pointer(gofixture.Named("User")))
			f.Return(gofixture.Named("error"))
		})
}
...
func TestBuilder_GoSource(t *testing.T) {
	...
	t.Run("follows declarations repointed at the module root", func(t *testing.T) {
		...
		// identifiers.
		path, _ := gofixture.New().
			Struct("Article", func(s *gofixture.StructBuilder) {
	...
	t.Run("refuses declarations spread across directories", func(t *testing.T) {
		...
		requireUnspellable(t, "spread across 2 directories", "the fixture package", func() {
			gofixture.New().
				Struct("Here", nil).
				Struct("There", func(s *gofixture.StructBuilder) {
	...
	t.Run("follows a rename in the fixture rather than going stale", func(t *testing.T) {
		...
		_, src := gofixture.New().
			Struct("User", func(s *gofixture.StructBuilder) {
				s.Field("Identifier", gofixture.Named("string"), nil)
			}).
			GoSource()
...
func TestBuilder_GoSource_Imports(t *testing.T) {
	...
	t.Run("aliases a second import whose last segment collides", func(t *testing.T) {
		...
		_, src := gofixture.New().
			Struct("Pair", func(s *gofixture.StructBuilder) {
				s.Field("A", gofixture.PkgNamed("example.com/one/models", "T"), nil)
				s.Field("B", gofixture.PkgNamed("example.com/two/models", "T"), nil)
			}).
			GoSource()
	...
	t.Run("drops an import nothing in the package references", func(t *testing.T) {
		...
		_, src := gofixture.New().Import("context").Struct("S", nil).GoSource()
	...
	t.Run("omits the import block when nothing needs one", func(t *testing.T) {
		t.Parallel()
		_, src := gofixture.New().Struct("S", nil).GoSource()
...
func TestBuilder_GoSource_Signatures(t *testing.T) {
	...
	t.Run("keeps an all-anonymous parameter list anonymous", func(t *testing.T) {
		t.Parallel()
		if got := methodSpecOf(t, func(m *gofixture.MethodBuilder) {
	...
	t.Run("composes a generic receiver with its type arguments", func(t *testing.T) {
		...
		_, src := gofixture.New().
			Struct("List", func(s *gofixture.StructBuilder) {
				s.TypeParam("T", nil)
				s.Method("Len", func(m *gofixture.MethodBuilder) {
					m.ReceiverName("l")
	...
	t.Run("honours a value receiver the fixture pinned", func(t *testing.T) {
		...
		_, src := gofixture.New().
			Struct("User", func(s *gofixture.StructBuilder) {
				s.Method("String", func(m *gofixture.MethodBuilder) {
					m.ReceiverName("u")
					m.Receiver(gofixture.PkgNamed("example.com/test", "User"))
					m.Return(gofixture.Named("string"))
				})
			}).
			GoSource()
...
func TestBuilder_GoSource_Refuses(t *testing.T) {
	...
	t.Run("a field with no type", func(t *testing.T) {
		...
		requireUnspellable(t, "nil type reference", "field ID", func() {
			gofixture.New().Struct("User", func(s *gofixture.StructBuilder) {
				s.Field("ID", nil, nil)
			}).GoSource()
		})
	})
	...
	t.Run("a field with no name", func(t *testing.T) {
		...
		requireUnspellable(t, "field with no name", "struct User", func() {
			gofixture.New().Struct("User", func(s *gofixture.StructBuilder) {
				s.Field("", gofixture.Named("string"), nil)
			}).GoSource()
		})
	})
	...
	t.Run("a variadic parameter that is not last", func(t *testing.T) {
		...
		requireUnspellable(t, "variadic parameter that is not the last", "method M", func() {
			gofixture.New().Interface("I", func(i *gofixture.InterfaceBuilder) {
				i.Method("M", func(m *gofixture.MethodBuilder) {
					m.Variadic("opts", gofixture.Named("string"))
					m.Param("id", gofixture.Named("string"))
				})
			}).GoSource()
	...
	t.Run("a type parameter on a method", func(t *testing.T) {
		...
		requireUnspellable(t, "only on the receiver type", "method M", func() {
			gofixture.New().Struct("S", func(s *gofixture.StructBuilder) {
				s.Method("M", func(m *gofixture.MethodBuilder) {
					m.TypeParam("T", nil)
				})
			}).GoSource()
		})
	...
	t.Run("a receiver naming a type other than its owner", func(t *testing.T) {
		...
		requireUnspellable(t, "naming Other rather than User", "method M", func() {
			gofixture.New().Struct("User", func(s *gofixture.StructBuilder) {
				s.Method("M", func(m *gofixture.MethodBuilder) {
					m.Receiver(gofixture.Pointer(
						gofixture.PkgNamed("example.com/test", "Other"),
					))
				})
			}).GoSource()
		})
	...
	t.Run("an enum with no underlying type", func(t *testing.T) {
		...
		requireUnspellable(t, "no underlying type", "enum Status", func() {
			gofixture.New().Enum("Status", func(e *gofixture.EnumBuilder) {
	...
	t.Run("a type declaration with no target", func(t *testing.T) {
		...
		requireUnspellable(t, "no target type", "type Handle", func() {
			gofixture.New().Alias("Handle", nil).GoSource()
		})
	})
	...
	t.Run("a constant with no value", func(t *testing.T) {
		...
		requireUnspellable(t, "constant with no value", "constant Max", func() {
			gofixture.New().Constant("Max", nil).GoSource()
		})
	})
	...
	t.Run("a variable with neither a type nor an initialiser", func(t *testing.T) {
		...
		requireUnspellable(t, "neither a type nor an initialiser", "variable Reg", func() {
			gofixture.New().Variable("Reg", nil).GoSource()
		})
	})
	...
	t.Run("a package with no name", func(t *testing.T) {
		...
		requireUnspellable(t, "package with no name", "the fixture package", func() {
			gofixture.New().Package("", "example.com/test").GoSource()
		})
	})
...
func methodSpecOf(t *testing.T, fn func(*gofixture.MethodBuilder)) string {
	t.Helper()
	_, src := gofixture.New().
		Interface("I", func(i *gofixture.InterfaceBuilder) { i.Method("M", fn) }).
		GoSource()
	return src
}

// requireUnspellable asserts that fn stops with an
// [gofixture.UnspellableError] naming both the construct and where
// it was found.
//
...
func requireUnspellable(t *testing.T, construct, where string, fn func()) {
	...
	defer func() {
		...
		var unspellable *gofixture.UnspellableError
		if !errors.As(err, &unspellable) {
			t.Fatalf("panicked with %T (%v); want *gofixture.UnspellableError", err, err)
		}
...
func TestGoSource_TextRefsKeepTheirImports(t *testing.T) {
	...
	t.Run("a variable initialiser keeps its import", func(t *testing.T) {
		t.Parallel()
		b := gofixture.New().Package("cfg", "example.com/cfg")
		b.Import("errors")
		b.Variable("ErrEmpty", func(v *gofixture.VariableBuilder) {
			v.Type(gofixture.Named("error"))
	...
	t.Run("a constant value keeps its import", func(t *testing.T) {
		...
		b := gofixture.New().Package("cfg", "example.com/cfg")
		b.Import("time")
		b.Constant("Timeout", func(c *gofixture.ConstantBuilder) {
			c.Value("time.Second")
		})
		_, src := b.GoSource()
	...
	t.Run("an aliased import is matched by its alias", func(t *testing.T) {
		...
		b.Variable("Default", func(v *gofixture.VariableBuilder) {
	...
	t.Run("an undeclared import is not invented", func(t *testing.T) {
		...
		b := gofixture.New().Package("cfg", "example.com/cfg")
		b.Import("errors")
		b.Variable("Bad", func(v *gofixture.VariableBuilder) {
			v.Type(gofixture.Named("error"))
	...
	t.Run("a qualifier inside a string literal is not a reference", func(t *testing.T) {
		...
		b := gofixture.New().Package("cfg", "example.com/cfg")
		b.Import("errors")
		b.Variable("ErrPath", func(v *gofixture.VariableBuilder) {
			v.Type(gofixture.Named("error"))
	...
	t.Run("only the head of a selector chain names a package", func(t *testing.T) {
		...
		// or, worse, refuses.
		b := gofixture.New().Package("app", "example.com/app")
		b.Import("example.com/cfg")
		b.Variable("T", func(v *gofixture.VariableBuilder) {
	...
	t.Run("a declared import named inside a literal is not activated", func(t *testing.T) {
		...
		b.Import("time")
		b.Import("errors")
		b.Variable("Timeout", func(v *gofixture.VariableBuilder) {
...
func TestUnspellableError_Error(t *testing.T) {
	t.Parallel()

	// Every other refusal test reads Construct and Where off the
	// struct, so the sentence an author actually sees was the one
	// thing none of them checked.
	err := &gofixture.UnspellableError{Where: "field X", Construct: "kind chan"}

	t.Run("names both the construct and where it was found", func(t *testing.T) {
		t.Parallel()
		msg := err.Error()
		if !strings.Contains(msg, "kind chan") {
			t.Errorf("message does not name the construct: %s", msg)
		}
		if !strings.Contains(msg, "field X") {
			t.Errorf("message does not name the declaration: %s", msg)
		}
	})

	t.Run("carries the package prefix the repo's sentinels use", func(t *testing.T) {
		t.Parallel()
		if !strings.HasPrefix(err.Error(), "gofixture: ") {
			t.Errorf("message = %q, want the gofixture: prefix", err.Error())
		}
	})

	t.Run("says what to do instead", func(t *testing.T) {
		t.Parallel()
		// The refusal is a design position, not a defect report: the
		// author is meant to hand-write the support file rather than
		// wait for the projection to grow a case.
		if !strings.Contains(err.Error(), "hand-write") {
			t.Errorf("message offers no remedy: %s", err.Error())
		}
	})
}
