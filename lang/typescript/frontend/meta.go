...
//
// Re-exported from [node] rather than declared here, and the one
// exception to the `ts.*` namespace: it carries the bare name
// `frontend` because it is a cross-frontend convention rather than a
// TypeScript fact. Every frontend used to declare
...
//
//nolint:gochecknoglobals // re-exported registry singleton
