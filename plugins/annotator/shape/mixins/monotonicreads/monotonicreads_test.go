...
func TestMixin_VersionResolvesAgainstTheValue(t *testing.T) {
		...
		host := &sdk.Function{
			...
			BaseNode: sdk.BaseNode{
				DirectiveList: []*sdk.Directive{
					mixintest.HostDirective(monotonicreads.Name, map[string]string{
						monotonicreads.ParamVersion: field,
					}),
				},
			},
		}
	...
	t.Run("a declared field rewrites qualified", func(t *testing.T) {
		...
		for _, d := range mixintest.RunWithValidator(t, monotonicreads.Mixin(), pkg) {
			if d.Severity == sdk.SeverityError {
				t.Fatalf("unexpected diagnostic: %s", d.Message)
			}
		}
		got, _ := shape.MixinParamKey(monotonicreads.Name, monotonicreads.ParamVersion).Get(host.Meta())
	...
	t.Run("a field the value does not declare is reported", func(t *testing.T) {
		...
		_, pkg := build("Missing")
		diags := mixintest.RunWithValidator(t, monotonicreads.Mixin(), pkg)
