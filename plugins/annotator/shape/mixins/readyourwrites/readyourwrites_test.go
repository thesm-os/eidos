...
func TestMixin_VersionResolvesAgainstTheValue(t *testing.T) {
			...
			BaseNode: sdk.BaseNode{
				DirectiveList: []*sdk.Directive{
					mixintest.HostDirective(readyourwrites.Name, map[string]string{
						readyourwrites.ParamVersion: field,
					}),
				},
			},
		}
	...
	t.Run("a declared field rewrites qualified", func(t *testing.T) {
		...
		for _, d := range mixintest.RunWithValidator(t, readyourwrites.Mixin(), pkg) {
	...
	t.Run("a field the value does not declare is reported", func(t *testing.T) {
		...
		_, pkg := build("Missing")
		diags := mixintest.RunWithValidator(t, readyourwrites.Mixin(), pkg)
